...
function encrypt() {
   ...
   encryptedText = text.replace(/e/g, "enter").replace(/i/g, "imes").replace(/a/g, "ai").replace(/o/g, "ober").replace(/u/g, "ufat");
...
function decrypt() {
   hideDecryptInfo();
   getText(textToEncrypt);
   encryptedText = text.replace(/enter/g, "e").replace(/imer/g, "i").replace(/ai/g, "a").replace(/ober/g, "o").replace(/ufat/g, "u");
   insertText(textEncryptedArea, encryptedText);
}
